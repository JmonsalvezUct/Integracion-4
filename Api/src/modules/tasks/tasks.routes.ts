...
import {
  ...
  assignTask,
  changeStatus,
  exportTasks, // <-- Importar controlador
...
/**
 * @swagger
 * /tasks/projects/{projectId}/export:
 * post:
 * summary: Exportar tareas de un proyecto
 * description: >
 * Exporta tareas en formato CSV o PDF.
 * Permite seleccionar tareas manualmente (usando 'taskIds') o 
 * mediante filtros (usando 'filters').
 * Si no se provee 'taskIds' ni 'filters', se exportan todas las tareas del proyecto.
 * tags: [Tasks]
 * security:
 * - bearerAuth: []
 * parameters:
 ...
 * items:
 * type: integer
 * description: Lista de IDs de tareas para selección manual (ignora filtros si se usa).
 * example: [1, 5, 12]
 * filters:
 * type: object
 * description: Filtros a aplicar si no se usa 'taskIds'.
 * properties:
 * status:
 * type: string
 ...
 * assigneeId:
 * type: integer
 * description: ID del usuario asignado (usar 0 para "sin asignar").
 * sprintId:
 * type: integer
 * description: ID del sprint (usar 0 para "sin sprint").
 * responses:
 * 200:
 * description: Archivo (CSV o PDF) de tareas exportadas.
 * content:
 * application/pdf:
 * schema:
 * type: string
 ...
 * format: binary
 * 400:
 * description: Datos inválidos (ej. formato incorrecto)
 * 401:
 * description: No autorizado
 * 403:
 * description: No tiene permisos en el proyecto
 * 404:
 * description: Proyecto o tareas no encontradas
 * x-middleware:
 * - authMiddleware
 */
router.post('/projects/:projectId/export', authMiddleware, rbacMiddleware(['admin', 'developer', 'guest']), exportTasks);
