...
export function useTasks(projectId?: string | number) {
  ...
  // 🔥 NUEVA FUNCIÓN: Actualizar fecha de tarea
  const updateTaskDate = async (taskId: number, newDate: Date) => {
    try {
      console.log(`🔄 Actualizando tarea ${taskId} a fecha: ${newDate.toISOString()}`);
      
      const token = await getAccessToken();
      
      // Usamos el endpoint de tu documentación: PUT /tasks/projects/(projectId)/tasks/(taskId)
      const res = await apiFetch(`/tasks/projects/${projectId}/tasks/${taskId}`, {
        method: "PUT",
        headers: { 
  ...
  return {
      ...
      fetchTasks,
      updateTaskDate, // 🔥 NUEVO: Exportamos la función de actualización de fecha
  };
}